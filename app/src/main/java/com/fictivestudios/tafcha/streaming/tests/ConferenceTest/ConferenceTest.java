package com.fictivestudios.tafcha.streaming.tests.ConferenceTest;
...
public class ConferenceTest extends PublishTest {
	...
	// Use hardware to test with headphone+mic.
	public boolean pubHW = true;
	public boolean subHW = false;
	...
	public void publish() {
		...
		config.setContextName(TestContent.GetPropertyString("context") + "/" + roomName);
		...
		if(pubHW){
			pack.stream.audioController.setHardwareEnabled(pubHW, getContext());
		}
...
//        pack.view.showDebugView(TestContent.GetPropertyBool("debug_view"));
	...
	public void Subscribe(String toName) {
		...
		if(subHW){
			pack.stream.audioController.setHardwareEnabled(subHW, getContext());
		}
	...
	public void clearByName(String clearName) {
		Log.d("[TEST]", "clearByName() : " + clearName);
...
//        pack.view.showDebugView(TestContent.GetPropertyBool("debug_view"));
	...
	public R5VideoView makeNewView() {
		...
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.MATCH_PARENT);
		params.weight = 1;
		r5view.setBackgroundColor(0xFFAAAAAA);
